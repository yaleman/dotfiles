Create Codex pet row `running-right` for `maltilda`: exactly 8 full-body frames in one horizontal strip on flat pure green #00FF00.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Preserve the existing Maltilda identity exactly: petite white Maltese terrier, silky layered coat, dark almond eyes, black button nose, poised slightly sassy expression, realistic soft 3D illustration.. Preserve silhouette, face, palette, material, proportions, markings, and props.
